...
function addPushSubscription(userId, subscription) {
  ...
  // Remove duplicate endpoints for this user
  const sub = JSON.parse(subscription);
  d.pushSubscriptions = d.pushSubscriptions.filter(s => {
    try { return !(s.user_id === userId && JSON.parse(s.subscription).endpoint === sub.endpoint); }
    catch(e) { return true; }
  });
  d.pushSubscriptions.push({ id: Date.now(), user_id: userId, subscription, created_at: new Date().toISOString() });
  save();
}

function getPushSubscriptions(userId) {
  const d = get();
  if (!d.pushSubscriptions) return [];
  return d.pushSubscriptions.filter(s => s.user_id === userId);
}

function removePushSubscription(id) {
  const d = get();
  if (!d.pushSubscriptions) return;
  d.pushSubscriptions = d.pushSubscriptions.filter(s => s.id !== id);
  save();
}
