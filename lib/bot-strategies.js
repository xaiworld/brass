...
function extractFeatures(state, player, action) {
  ...
  // Money situation
  features.money_ratio = player.money / 50; // normalized
  ...
  if (action.type === 'buildIndustry') {
    ...
    features.tile_cost_ratio = tileData ? tileData.cost / player.money : 1;
  ...
  if (action.type === 'buildLink') {
    ...
    if (link) {
      // Check if endpoints have flipped tiles (VP potential for scoring)
      let endpointVP = 0;
  ...
  if (action.type === 'develop') {
    ...
    // Developing is good when stuck with low level tiles in rail era
    let lowLevelCount = 0;
    if (action.develops) {
      for (const type of action.develops) {
        const mat = player.industryMat[type];
        if (mat && mat.length > 0 && mat[0] <= 1) lowLevelCount++;
      }
    }
...
class DRLBot {
  ...
  static featureNames() {
    return [
      ...
      'link_endpoint_vp', 'link_segments',
      'sell_vp', 'sell_via_port', 'sell_count',
      'loan_amount', 'loan_need',
      'develop_count', 'develop_skips_low'
    ];
  }
