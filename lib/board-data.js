...
// 2-player: remove 1 x(-1), 1 x(-3), 2 x(0) → 7 tiles
const distantMarketTileValues2P = [-2, -2, -2, -2, -3, -3, -4];
...
// 2-player: 6 slots, prices: [2, 2, 3, 3, 4, 4]
const MARKET_SLOTS_2P = 6;
...
const marketSlotPrices2P = [2, 2, 3, 3, 4, 4]; // 2-player pricing

function getMarketBuyPrice(cubesInMarket, numPlayers) {
  const slots = (numPlayers === 2) ? MARKET_SLOTS_2P : MARKET_SLOTS;
  const prices = (numPlayers === 2) ? marketSlotPrices2P : marketSlotPrices;
  if (cubesInMarket <= 0) return 5; // empty market, buy from bank
  const cheapestFilledSlot = slots - cubesInMarket;
  return prices[cheapestFilledSlot] || 5;
}

function getMarketSellPrice(cubesInMarket, numPlayers) {
  const slots = (numPlayers === 2) ? MARKET_SLOTS_2P : MARKET_SLOTS;
  const prices = (numPlayers === 2) ? marketSlotPrices2P : marketSlotPrices;
  if (cubesInMarket >= slots) return 0; // market full, can't sell
  // Selling fills from most expensive slot down
  // cubesInMarket=0 → fill slot 7 (£4), cubesInMarket=1 → fill slot 6 (£4), etc.
  const slotToFill = slots - 1 - cubesInMarket;
  return prices[slotToFill] || 0;
}
