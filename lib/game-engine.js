...
function actionBuildIndustry(state, player, action) {
  ...
  const incChange = player.income !== incomeBefore ? `, inc ${incomeBefore}→${player.income}` : '';
...
function actionSellCotton(state, player, action) {
  ...
  for (const sale of sales) {
    ...
    if (sale.target.type === 'port') {
      ...
      state.log.push(logMsg(state, `${player.username} sold cotton from ${locations[sale.millLocation].name} via port at ${locations[sale.target.location].name} (mill owner inc→${state.players[millSlot.owner].income}, port owner inc→${state.players[portSlot.owner].income})`));
      ...
      state.log.push(logMsg(state, `${player.username} sold cotton to distant market (demand: ${state.distantMarketDemand}, inc ${player.income})`));
    }
  }
...
function actionTakeLoan(state, player, action) {
  ...
  removeCard(player, cardPlayed);
  state.log.push(logMsg(state, `${player.username} took a loan of £${loanAmount} (has £${player.money}, inc ${incBefore}→${player.income})`));
  return { success: true };
}
