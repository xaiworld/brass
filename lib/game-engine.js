...
const { locations, links, coalMarketPrices, ironMarketPrices, incomeTrack, externalPorts } = require('./board-data');
...
function isConnectedToPort(state, locId) {
  // Check if location is connected to any port (built port tile or external port)
  ...
  for (const cLoc of connected) {
    ...
    // Check for external port connections (Scotland, Yorkshire, The Midlands)
    for (const ep of Object.values(externalPorts)) {
      if (ep.connectedTo.includes(cLoc)) return true;
    }
  }
  return false;
}
