...
function actionBuildIndustry(state, player, action) {
  ...
  if (player.income !== incomeBefore) {
    ...
    incChange = `, inc +${moved} sq: ${incStr(incomeBefore)}/turn→${incStr(player.income)}/turn`;
  }
...
function actionSellCotton(state, player, action) {
  ...
  for (const sale of sales) {
    ...
    if (sale.target.type === 'port') {
      ...
      state.log.push(logMsg(state, `${player.username} sold cotton from ${locations[sale.millLocation].name} via port at ${locations[sale.target.location].name} (mill owner: ${incStr(state.players[millSlot.owner].income)}/turn, port owner: ${incStr(state.players[portSlot.owner].income)}/turn)`));
      ...
      state.log.push(logMsg(state, `${player.username} sold cotton to distant market (demand: ${state.distantMarketDemand}, ${incStr(player.income)}/turn)`));
    }
  }
