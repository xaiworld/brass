...
function actionBuildLink(state, player, action) {
  ...
  if (state.era === 'canal') {
    ...
    if (secondLinkId) {
      ...
      // Need coal for each segment — can transport to either end of each link
      ...
      if (!coalEnd2) return { error: 'Cannot transport coal to second link' };
      ...
      state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] built 2 rail links (spent £${totalMoney}, has £${player.money})`));
