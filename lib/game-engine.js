...
function actionBuildLink(state, player, action) {
  ...
  if (state.era === 'canal') {
    ...
    state.log.push({ msg: `${player.username} built canal ${locName(link.from)} - ${locName(link.to)}${throughNote}` });
  } else {
    // Rail era
    ...
    if (secondLinkId) {
      ...
      const throughNote = link.through ? ` (via ${link.through})` : '';
      state.log.push({ msg: `${player.username} built rail ${locName(link.from)} - ${locName(link.to)}${throughNote}` });
    }
  }
