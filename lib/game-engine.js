...
function actionBuildIndustry(state, player, action) {
  ...
  if (wildBuild && secondCard) {
    ...
    state.actionsRemaining--; // consume extra action
  }
  ...
  const incChange = player.income !== incomeBefore ? `, inc ${incStr(incomeBefore)}→${incStr(player.income)}` : '';
  const wildNote = wildBuild ? ' [WILD]' : '';
  state.log.push(logMsg(state, `${player.username} built ${industries[industryType].name} L${level} at ${locations[location].name}${wildNote} (spent £${totalCost}, has £${player.money}${incChange})`));
  return { success: true };
}
...
function actionSellCotton(state, player, action) {
  ...
  for (const sale of sales) {
    ...
    if (sale.target.type === 'port') {
      ...
      state.log.push(logMsg(state, `${player.username} sold cotton from ${locations[sale.millLocation].name} via port at ${locations[sale.target.location].name} (mill inc→${incStr(state.players[millSlot.owner].income)}, port inc→${incStr(state.players[portSlot.owner].income)})`));
      ...
      state.log.push(logMsg(state, `${player.username} sold cotton to distant market (demand: ${state.distantMarketDemand}, inc→${incStr(player.income)})`));
    }
  }
...
function actionTakeLoan(state, player, action) {
  ...
  removeCard(player, cardPlayed);
  state.log.push(logMsg(state, `${player.username} took a loan of £${loanAmount} (has £${player.money}, inc ${incStr(incBefore)}→${incStr(player.income)})`));
  return { success: true };
}
