...
function cardName(cardId) {
  ...
  if (info.type === 'location') {
    return (locations[info.location]?.name || info.location) + ' card';
  }
  return (industries[info.industry]?.name || info.industry) + ' card';
}
...
function dropIncomeLevel(player) {
  // Drop one income LEVEL (not square). Land on the highest square of the new level.
  const currentLevel = incomeTrack[player.income];
  if (currentLevel === undefined) return;
  const targetLevel = currentLevel - 1;
  // Find the first (highest) square where incomeTrack[sq] === targetLevel
  let targetSquare = 0; // default to square 0 (-10)
  for (let sq = 0; sq < incomeTrack.length; sq++) {
    if (incomeTrack[sq] === targetLevel) {
      targetSquare = sq;
      break; // first square of that level = highest position
    }
  }
  player.income = Math.max(0, targetSquare);
}
...
function findAndConsumeIron(state, amount) {
  ...
  const costs = { money: 0, flipped: [], log: [] };

  // First: take from board tiles (free)
  for (const [locId, loc] of Object.entries(state.board.locations)) {
    for (let i = 0; i < loc.slots.length; i++) {
      ...
      if (slot.industryType === 'ironWorks' && !slot.flipped && slot.resources > 0) {
        ...
        const locN = locations[locId]?.name || locId;
        slot.resources -= take;
        remaining -= take;
        costs.log.push(take + ' iron from ' + locN + ' (free, ' + slot.resources + ' left)');
        if (slot.resources === 0) {
          slot.flipped = true;
          const levelData = industries.ironWorks.levels[slot.level];
          const incBefore = state.players[slot.owner].income;
          adjustIncome(state.players[slot.owner], levelData.incomeGain);
          costs.flipped.push({ loc: locId, slot: i, owner: slot.owner });
          costs.log.push('Iron Works L' + slot.level + ' at ' + locN + ' flipped! ' + state.players[slot.owner].username + ' inc +' + levelData.incomeGain + 'sq: ' + incStr(incBefore) + '→' + incStr(state.players[slot.owner].income) + '/turn, ' + levelData.vp + 'VP');
        }
  ...
  if (remaining > 0 && state.ironMarket > 0) {
    ...
    for (let i = 0; i < take; i++) {
      marketCost += getMarketBuyPrice(state.ironMarket, state.numPlayers);
      state.ironMarket--;
    }
    costs.money += marketCost;
    remaining -= take;
    costs.log.push(take + ' iron from market (£' + marketCost + ')');
  }

  // Then: take from bank at price 5
  if (remaining > 0) {
    const bankCost = remaining * 5;
    costs.money += bankCost;
    costs.log.push(remaining + ' iron from bank (£' + bankCost + ')');
    remaining = 0;
  }
...
function findAndConsumeCoal(state, targetLoc, amount) {
  ...
  // First: take from connected board tiles (free)
  for (const cLoc of connected) {
    const loc = state.board.locations[cLoc];
    if (!loc) continue;
    for (let i = 0; i < loc.slots.length; i++) {
      ...
      if (slot.industryType === 'coalMine' && !slot.flipped && slot.resources > 0) {
        const take = Math.min(remaining, slot.resources);
        const locN = locations[cLoc]?.name || cLoc;
        slot.resources -= take;
        remaining -= take;
        costs.log.push(take + ' coal from ' + locN + ' (free, ' + slot.resources + ' left)');
        if (slot.resources === 0) {
          slot.flipped = true;
          ...
          costs.log.push('Coal Mine L' + slot.level + ' at ' + locN + ' flipped! ' + state.players[slot.owner].username + ' inc +' + levelData.incomeGain + 'sq: ' + incStr(incBefore) + '→' + incStr(state.players[slot.owner].income) + '/turn, ' + levelData.vp + 'VP');
        }
        if (remaining === 0) return costs;
      }
    }
  ...
  if (remaining > 0 && isConnectedToPort(state, targetLoc)) {
    if (state.coalMarket > 0) {
      ...
      let marketCost = 0;
      for (let i = 0; i < take; i++) {
        marketCost += getMarketBuyPrice(state.coalMarket, state.numPlayers);
        state.coalMarket--;
      }
      costs.money += marketCost;
      remaining -= take;
      costs.log.push(take + ' coal from market (£' + marketCost + ')');
    }
    if (remaining > 0) {
      const bankCost = remaining * 5;
      costs.money += bankCost;
      costs.log.push(remaining + ' coal from bank (£' + bankCost + ')');
      remaining = 0;
    }
  }
...
function actionBuildIndustry(state, player, action) {
  ...
  if (tileData.ironCost > 0) {
    ...
    resourceLogs.push(...ironResult.log);
  }
  ...
  if (tileData.coalCost > 0) {
    ...
    resourceLogs.push(...coalResult.log);
  }
  ...
  if (industryType === 'ironWorks' && slot.resources > 0) {
    while (slot.resources > 0 && state.ironMarket < (state.marketSlots || MARKET_SLOTS)) {
      const sellPrice = getMarketSellPrice(state.ironMarket, state.numPlayers);
      slot.resources--;
      state.ironMarket++;
  ...
  if (industryType === 'coalMine' && slot.resources > 0 && isConnectedToPort(state, location)) {
    while (slot.resources > 0 && state.coalMarket < (state.marketSlots || MARKET_SLOTS)) {
      const sellPrice = getMarketSellPrice(state.coalMarket, state.numPlayers);
      slot.resources--;
      state.coalMarket++;
      player.money += sellPrice;
      cubesSold++;
  ...
  state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] built ${industries[industryType].name} L${level} at ${locations[location].name}${wildNote} (${logParts.join(', ')})`));
  // Resource consumption details
  for (const rl of resourceLogs) {
    state.log.push(logMsg(state, '  ↳ ' + rl));
  }
  return { success: true };
}
...
function actionBuildLink(state, player, action) {
  ...
  if (state.era === 'canal') {
    ...
    state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] built canal ${locName(link.from)} - ${locName(link.to)}${throughNote} (spent £${canalCost}, has £${player.money})`));
  } else {
    // Rail era
    if (!link.rail) return { error: 'This link is not available in Rail era' };

    if (secondLinkId) {
      // Building 2 links in one action: £15 + 2 coal
      ...
      // 1) Find coal for first link (check both ends)
      const test1From = findAndConsumeCoal(clone(state), link.from, seg);
      const test1To = !test1From ? findAndConsumeCoal(clone(state), link.to, seg) : null;
      const coalEnd1 = test1From ? link.from : test1To ? link.to : null;
      if (!coalEnd1) return { error: 'Cannot transport coal to first link' };
      const coalCost1 = findAndConsumeCoal(state, coalEnd1, seg);

      // 2) Place first rail BEFORE finding coal for second (so its connection is available)
      link.owner = player.seat;
      link.type = 'rail';

      // 3) Find coal for second link (can now use first rail's connection)
      const test2From = findAndConsumeCoal(clone(state), link2.from, seg2);
      const test2To = !test2From ? findAndConsumeCoal(clone(state), link2.to, seg2) : null;
      const coalEnd2 = test2From ? link2.from : test2To ? link2.to : null;
      if (!coalEnd2) {
        // Undo first rail if second fails
        link.owner = null;
        link.type = null;
        return { error: 'Cannot transport coal to second link' };
      }
      const coalCost2 = findAndConsumeCoal(state, coalEnd2, seg2);

      // 4) Place second rail, charge money
      ...
      if (player.money < totalMoney) {
        // Undo both rails
        link.owner = null; link.type = null;
        link2.owner = null; link2.type = null;
      ...
      const name1 = locName(link.from) + '—' + locName(link.to);
      const name2 = locName(link2.from) + '—' + locName(link2.to);
      state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] built 2 rails: ${name1} + ${name2} (spent £${totalMoney}, has £${player.money})`));
      for (const rl of [...coalCost1.log, ...coalCost2.log]) state.log.push(logMsg(state, '  ↳ ' + rl));
    } else {
      // Building 1 link
      ...
      // Test with clone first to avoid state corruption
      const testFrom = findAndConsumeCoal(clone(state), link.from, seg);
      const testTo = !testFrom ? findAndConsumeCoal(clone(state), link.to, seg) : null;
      const coalEnd = testFrom ? link.from : testTo ? link.to : null;
      if (!coalEnd) return { error: 'Cannot transport coal to link' };
      const coalCost = findAndConsumeCoal(state, coalEnd, seg);
      ...
      const throughNote = link.through ? ` (via ${link.through})` : '';
      state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] built rail ${locName(link.from)} - ${locName(link.to)}${throughNote} (spent £${totalMoney}, has £${player.money})`));
      for (const rl of coalCost.log) state.log.push(logMsg(state, '  ↳ ' + rl));
    }
  }
...
function actionSellCotton(state, player, action) {
  ...
  for (const sale of sales) {
    ...
    if (sale.target.type === 'port') {
      ...
      state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] sold cotton from ${locations[sale.millLocation].name} via port at ${locations[sale.target.location].name} (${millInfo}, ${portInfo})`));
      ...
      state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] sold cotton to distant market (demand: ${state.distantMarketDemand}, mill +${millData.incomeGain}sq +${demandBonus}bonus: ${incStr(distIncBefore)}→${incStr(distMillOwner.income)}/turn)`));
    }
  }
...
function actionTakeLoan(state, player, action) {
  ...
  for (let i = 0; i < bands; i++) {
    dropIncomeLevel(player);
  }

  removeCard(player, cardPlayed);
  state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] took a loan of £${loanAmount} (has £${player.money}, inc ${incStr(incBefore)}→${incStr(player.income)}/turn)`));
  return { success: true };
}
...
function actionDevelop(state, player, action) {
  ...
  for (const industryType of develops) {
    ...
    state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] developed ${industries[industryType].name} L${removed} (spent £${actualIronResult.money}, has £${player.money})`));
  }
  // Iron consumption details
  for (const rl of actualIronResult.log) {
    state.log.push(logMsg(state, '  ↳ ' + rl));
  }
...
function actionPass(state, player, action) {
  ...
  if (!hasCard(player, cardPlayed)) return { error: 'You don\'t have that card' };
  removeCard(player, cardPlayed);
  state.log.push(logMsg(state, `${player.username} [${cardName(cardPlayed)}] passed`));
  return { success: true };
}
...
function endRound(state) {
  ...
  // Phase 1: Collect income at START of new round
  for (const player of state.players) {
