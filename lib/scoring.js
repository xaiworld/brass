...
const { locations, nonBuildable, links } = require('./board-data');
...
function scoreCanalEra(state) {
  ...
  for (const link of Object.values(state.board.links)) {
    if (link.type === 'canal' && link.owner !== null) {
      ...
      if (linkVP > 0) {
        state.log.push({
          msg: `${state.players[link.owner].username} scores ${linkVP} VP from canal ${locName(link.from)}-${locName(link.to)}`
        });
      }
    }
  }
...
function scoreRailEra(state) {
  ...
  for (const link of Object.values(state.board.links)) {
    if (link.type === 'rail' && link.owner !== null) {
      ...
      if (linkVP > 0) {
        state.log.push({
          msg: `${state.players[link.owner].username} scores ${linkVP} VP from rail ${locName(link.from)}-${locName(link.to)}`
        });
      }
    }
  }
