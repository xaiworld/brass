...
const { getBotByTier, getBotByName, getTierAssignments } = require('./bot-strategies');
...
class NeuralNetBot extends BotStrategy {
  ...
  chooseAction(state, player) {
    ...
    const { getValidActions } = require('./game-engine');
    const { ValidActionGenerator } = require('./bot-strategies');
    ...
    try {
      const { chooseActionNN } = require('./nn-inference');
      const chosen = chooseActionNN(state, player, validActions, this.temperature);
      if (chosen) return chosen;
    } catch (e) {
      console.log('[NN Bot] Inference error:', e.message, '- falling back to DRL');
    }

    // Fallback to DRL bot
    const { getBotByTier } = require('./bot-strategies');
    const fallback = getBotByTier('pro');
    if (fallback) return fallback.chooseAction(state, player);
    return { type: 'pass', cardPlayed: hand[0] };
  }
}
...
const defaultStrategy = new HeuristicBot();
const nnBotPro = new NeuralNetBot(0.05);       // Near-greedy
const nnBotAverage = new NeuralNetBot(0.4);     // Some exploration
const nnBotNoob = new NeuralNetBot(1.0);        // High randomness

function getStrategyForBot(player) {
  // Check for neural network bot first
  if (player.botTier) {
    try {
      const { getNet } = require('./nn-inference');
      if (getNet()) {
        if (player.botTier === 'pro') return nnBotPro;
        if (player.botTier === 'average') return nnBotAverage;
        if (player.botTier === 'noob') return nnBotNoob;
      }
    } catch (e) { /* NN not available, fall through */ }

    // Fallback: trained DRL bots
    const { getTrainedBot } = require('./training-runner');
    const trained = getTrainedBot(player.botTier);
    if (trained) return trained;
    const tieredBot = getBotByTier(player.botTier);
    if (tieredBot) return tieredBot;
  }
  if (player.botStrategy) {
    const namedBot = getBotByName(player.botStrategy);
    if (namedBot) return namedBot;
  }
...
function playBotTurn(gameId) {
  ...
  // Choose action using appropriate strategy
  const strategy = getStrategyForBot(currentPlayer);
  ...
  if (state.phase === 'finished') {
    ...
    // Notify human players game finished
    try {
      const { notifyGameFinished } = require('./notifications');
      const winner = state.players.reduce((best, p) => p.vp > best.vp ? p : best, state.players[0]);
      const humanIds = state.players.filter(p => !p.isBot).map(p => p.userId);
  ...
  // Notify next human player if it's their turn (before checking for more bots)
  try {
    const nextSeat = state.turnOrder[state.currentPlayerIndex];
    const nextPlayer = state.players[nextSeat];
    if (nextPlayer && !nextPlayer.isBot) {
      const { notifyYourTurn } = require('./notifications');
      const game = db.findGame(gameId);
...
module.exports = { checkAndPlayBot, setBotDelay, getBotDelay, BotStrategy, HeuristicBot, describeAction, getStrategyForBot };
